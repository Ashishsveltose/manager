 ...
 <div class="mobile-menu-overlay"></div>
                            ...
                            <div class="col-md-12">
                              <!--<div class="col-6">-->
                                <div class="pd-20 float-right">
                                    <a href="<?php echo base_url();?>addevent" class="dropdown-toggle no-arrow">
   ...
   // $category1 = $this->Common_model->getSingleRecordById('article_user',array('id'=> $getdata['user_id']));
                  ...
                  ?>
                    ...
                    <td><?php  echo (!empty($getdata['event_name'])?$getdata['event_name']:'none'); ?></td>
                  <td><?php  echo (!empty($getdata['image'])?'<img src=https://invoice-o.com/artical_admin/upload/channels/'.$getdata['image'].' style="width:50px;height:50px">':'none'); ?></td>

<td>
    <?php
    $about_us = (!empty($getdata['about_us']) ? $getdata['about_us'] : 'none');
    // Split the text into words
    $words = explode(" ", $about_us);
    // Check if the number of words exceeds 50
    if (count($words) > 50) {
        // Join the first 50 words back together
        $trimmed_text = implode(" ", array_slice($words, 0, 50));
        echo $trimmed_text . " ..."; // Add ellipsis to indicate text has been trimmed
    } else {
        echo $about_us; // Display the original text if it contains 50 or fewer words
    }
    ?>
</td>
                        ...
                        <td>
                                                        ...
                                                        <div
                                                            ...
                                                            <a href="javascript:void(0)" href-data="<?php echo  $getdata['id']; ?>" class="dropdown-itemm" title="Delete"><i class="dw dw-delete-3" aria-hidden="true"></i>Delete</a>
                                                                                                                     <a style="margin-left:10px;"  href="<?php echo base_url() ?>welcome/addevent/<?php echo  $getdata['id']; ?>" title="Edit"><img src="<?php echo base_url();?>home/assets/vendors/images/edit.png" alt="" class="dark-logo" style="width:20px;height:20px;"/>Edit</a>
...
$(document).ready(function(){
  ...
  $(".dropdown-itemm").click(function(e){
    var val = confirm("Are you sure, you want to delete event ?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'article_event',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to event  ?");
        if (val == true){
     setTimeout(function(){location.reload()},1000);
    }
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'article_event',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate event ?");
    if (val == true){
       setTimeout(function(){location.reload()},1000);
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>welcome/change_status",
        data:{tablename:'article_event',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
