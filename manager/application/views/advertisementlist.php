            ...
            <div class="pd-ltr-20 xs-pd-20-10">
                <div class="min-height-200px">
                            ...
                            <div class="col-md-12">
                              <!--<div class="col-6">-->
                                <div class="pd-20 float-right">
                                    <a href="<?php echo base_url();?>addadvertisement" class="dropdown-toggle no-arrow">
                                        <button type="button" class="btn btn-primary">Add advertisement</button>
                                    </a> 
                              </div>
                              <!--   </div> -->
                  ...
                  ?>
                    ...
                    <td><?php  echo (!empty($getdata['content'])?$getdata['content']:'none'); ?></td>

 <td><?php  echo (!empty($getdata['image'])?'<img src=https://invoice-o.com/association/upload/channels/'.$getdata['image'].' style="width:50px;height:50px">':'none'); ?></td>
                        ...
                        <td>
                                                        ...
                                                        <div
                                                            ...
                                                            <a href="javascript:void(0)" href-data="<?php echo  $getdata['id']; ?>" class="dropdown-itemm" title="Delete"><i class="dw dw-delete-3" aria-hidden="true"></i>Delete</a>
                                                                                                                                            ...
                                                                                                                                            <a style="margin-left:10px;"  href="<?php echo base_url() ?>welcome/addadvertisement/<?php echo  $getdata['id']; ?>" title="Edit"><img src="<?php echo base_url();?>home/assets/vendors/images/edit.png" alt="" class="dark-logo" style="width:20px;height:20px;"/>Edit</a>
...
$(document).ready(function(){
  ...
  $(".dropdown-itemm").click(function(e){
    var val = confirm("Are you sure, you want to delete advertisement ?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'advertisement',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to deactivate annoucement ?");
        if (val == true){
      window.location.reload();
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'annoucement',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate annoucement ?");
    if (val == true){
      window.location.reload();
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>welcome/change_status",
        data:{tablename:'annoucement',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
