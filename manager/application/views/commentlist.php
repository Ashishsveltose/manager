...
<head>
 ...
 <div class="mobile-menu-overlay"></div>
    ...
    $category1 = $this->Common_model->getSingleRecordById('articles',array('id'=> $getdata['article_id']));
                  ...
                  ?>
                    ...
                    <td><?php  echo (!empty($category1['tittle'])?$category1['tittle']:'none'); ?></td>
                    <td><?php  echo (!empty($getdata['author'])?$getdata['author']:'none'); ?></td>
                    <td><?php  echo (!empty($getdata['content'])?$getdata['content']:'none'); ?></td>
                    <td>
                    <?php  echo (!empty($getdata['status'])?$getdata['status']:'none'); ?>
                          </td>
                        ...
                        <td>
                                                        ...
                                                        <div
                                                            ...
                                                            <a href="javascript:void(0)" href-data="<?php echo  $getdata['id']; ?>" class="dropdown-itemm" title="Delete"><i class="dw dw-delete-3" aria-hidden="true"></i>Delete</a>
                                                                                                                     <a style="margin-left:10px;"  href="<?php echo base_url() ?>welcome/editcomment/<?php echo  $getdata['id']; ?>" title="Edit"><img src="<?php echo base_url();?>home/assets/vendors/images/edit.png" alt="" class="dark-logo" style="width:20px;height:20px;"/>Edit</a>
...
$(document).ready(function(){
  ...
  $(".dropdown-itemm").click(function(e){
    var val = confirm("Are you sure, you want to delete article comment ?");
    if (val == true){
    ...
    if(val){
      $.ajax({
        ...
        data:{table:'article_comment',id:id,status:3,colwhr:'id',whrstatuscol:'status',action:"Delete"},
        dataType:'json',
  ...
  $(".deactive").click(function(e){
        var val = confirm("Are you sure, you want to article comment  ?");
        if (val == true){
     setTimeout(function(){location.reload()},1000);
    }
        ...
        if(val){
            $.ajax({
              ...
              data:{tablename:'article_comment',id:id,status:0,whrcol:'id',whrstatuscol:'status',action:"Deactive"},
              dataType:'json',
              success: function(response) {
                if (response.status == 1){
                  $.notify(response.msg, "success");
  ...
  $(".useractive").click(function(e){
    var val = confirm("Are you, sure you want to activate comment ?");
    if (val == true){
       setTimeout(function(){location.reload()},1000);
    }
    ...
    if(val){
      $.ajax({
        type: "POST",
        url: "<?php echo base_url();?>welcome/change_status",
        data:{tablename:'article_comment',id:id,status:1,whrcol:'id',whrstatuscol:'status',action:"Active"},
        dataType:'json',
        success: function(response) {
          if (response.status == 1){
            $.notify(response.msg, "success");
